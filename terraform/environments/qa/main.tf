...
#
# ---------------------------------------------------------------------------
# THIS ENVIRONMENT CANNOT BE APPLIED ON THE CURRENT SUBSCRIPTION.
#
# qa's profile is a genuine pre-production environment: a private two-node AKS
# cluster with a user node pool, Application Gateway WAF v2, Bastion Basic,
# SQL GP_Gen5_2 and Redis with high availability. That needs at least 6 vCPU
# against a regional quota of 4, of which dev already holds 2, and costs an
# order of magnitude more per month than the credit remaining.
#
# The configuration is complete and plans; it is not deployed. See README.md
# for the measurements and for what would have to change to run it.
# ---------------------------------------------------------------------------
################################################################################
...
# Phase 2 — network
#
# Address plan per docs/NETWORKING.md. qa occupies 10.20.0.0/16, spaced from
...
#
# Still reserved and not allocated: AzureFirewallSubnet 10.20.0.0/26,
# AzureFirewallManagementSubnet 10.20.0.64/26 and GatewaySubnet 10.20.0.192/26.
# qa egresses through a NAT Gateway for the same cost reason dev does.
################################################################################
...
#
# qa uses the NAT Gateway, so this table carries NO routes. A NAT Gateway
# attaches directly to a subnet and is NOT a UDR next hop — adding a 0.0.0.0/0
# route pointing anywhere would override it and break egress.
#
# The table is still created and attached, because disabling BGP route
# propagation is itself a control: without it, an ExpressRoute or VPN gateway
# attached later could advertise a more specific route that diverts egress,
# defeating the intended path with no change to this configuration.
#
# AzureBastionSubnet and the Application Gateway subnet are deliberately NOT
# associated. A default route on either breaks the service it hosts, and the
# module rejects the combination outright.
################################################################################
...
#
# qa's cluster is PRIVATE, which is the posture dev could not adopt.
#
...
#
# It is also HA, unlike dev: a two-node system pool across zones 1 and 2, plus
# an autoscaling user node pool that keeps application workload off the nodes
# running CoreDNS and the API server proxies. That is 6 vCPU at minimum, which
# is why this environment does not fit the current quota.
################################################################################
...
module "aks" {
  ...
  # both correct the moment it is not. The NAT Gateway address matters because
  # AKS only appends its own egress address to the allowlist when it owns the
  # outbound path, which with userAssignedNATGateway it does not.
...
module "diagnostics_aks" {
  ...
  # Empty in qa, unlike dev, which drops both kube-audit categories to survive
  # a 0.5 GB/day cap. qa is uncapped, so the API server audit trail is kept —
  # it is the log that answers "who did that" when reproducing a security
  # question, and a test environment is where that question gets asked.
