...
output "name_prefix" {
  description = "Shared name base, e.g. \"cloudcart-stage-cus\"."
...
output "log_ingestion_is_capped" {
  description = "Whether a daily ingestion cap is active. FALSE in stage: a cap stops ingestion once hit and drops everything after, which in a test environment means losing the evidence for whatever was being tested. Ingestion is billed here rather than capped."
...
# stage reports an inspection point.
################################################################################
