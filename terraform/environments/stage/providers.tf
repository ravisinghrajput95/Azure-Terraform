...
provider "azurerm" {
  ...
  features {
    ...
    key_vault {
      ...
      # qa keeps purge protection OFF (profile), for the same reason dev does:
      # the naming module produces a deterministic vault name, so protection
      # would block rebuilding the environment for the full retention period.
      # Purging on destroy is what makes that rebuild immediate.
